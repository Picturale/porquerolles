...
| 4 | Géorisques API v1 (risques, argiles, débroussaillement) | Risques | Archive + temps réel | Licence Ouverte — commercial OK | Granularité communale sauf `/rga`, ponctuel mais troué sur le littoral, et `/tri_zonage` (submersion marine), ponctuel et fiable — **`/old` n'est PAS ponctuel malgré son paramètre `latlon`, corrigé le 02/08/2026** |
...
- **Authentification** : aucune.
- **Couverture Porquerolles** : ponctuelle et fiable pour `/tri_zonage` (submersion marine — discrimine vraiment plage par plage) ; ponctuelle mais troué sur le littoral pour `/rga` (corps vide sur le bord de mer) ; **`/old` est en réalité communal** malgré son paramètre `latlon` — corrigé le 02/08/2026, voir `donnees/GEORISQUES-PORQUEROLLES.md` ; communale pour le reste.
- **Vérifié** : `/gaspar/risques?code_insee=83069` → HTTP 200 (feu de forêt, submersion marine, recul du trait de côte, mouvement de terrain...). `/old?latlon=6.21,43.00` → HTTP 200, mais renvoie le même enregistrement communal unique quel que soit le point interrogé dans la commune (arrêté approuvé le 2025-09-26). `/rga?latlon=6.21,43.00` → HTTP 200, exposition moyenne, mais corps vide sur de nombreux points côtiers testés. `/tri_zonage?latlon=6.2278,43.0100` → HTTP 200, aléa de forte probabilité à Notre-Dame, négatif à Argent et au Lequin — varie réellement point par point. `/zonage_sismique` → HTTP 200, « 2 - FAIBLE ». `/mvt` → HTTP 200, 11 événements pour la commune entière, **1 seul réellement sur Porquerolles** (Pointe du Bouvet — les 10 autres sont à Giens, au Levant, à Port-Cros ou à Hyères-ville, corrigé le 02/08/2026). `/gaspar/catnat` → HTTP 200, 28 arrêtés depuis 2002, dont 5 « chocs mécaniques liés à l'action des vagues ».
